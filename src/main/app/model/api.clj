...
;(def resources-m (memoize resources))
      ...
      (let [resources
            ...
            (fn
              ...
              (let [resource-server (first (filter #(= (:resource/id %)  id) resources))
                    #_val  #_(d/entity (d/db (d/connect db-url)) [:resource/id  id])]

                (if (number? (:db/id id)) id (remove-nils resource-server))))]
        (mapv (fn [x] (let [task-name (:task/name x)
                           task-id (:task/id x)
...
(defn get-all-projects
  ...
  (let [response (-> (client/get "https://flu.sharepoint.com/sites/pwa/_api/ProjectData/%5Ben-us%5D/Projects"
    ...
    (->> response
         ...
         #_(take 2)
         (map #(s/rename-keys % {:Name :project/name :Id :project/id}) )
         ...
         (mapv
          (fn [x] (remove-nils (assoc x
                                 :project/assignments (map (fn [x] (dissoc x :project/name))
                                                            #_(assignement-phased-project-id (str (:project/id x)) 0)
...
(comment
  ...
  (map (comp (fn [m] (update m :action-list/actions (fn [x] (map #(clojure.set/rename-keys %  {:db/id :action/id}) x)))) first)
        )
  ...
  (d/transact (d/connect db-url) (into [] (map (fn [e] [:db/retractEntity e]) (d/q '[:find [?e ...]
...
;; test2
  (d/transact (d/connect db-url)
              (into [] (map (fn [e] [:db/retractEntity e]) (d/q '[:find [?e ...]
...
#_(let [max 2000
      url (str "https://flu.sharepoint.com/sites/pwa/_api/ProjectData/%5Ben-us%5D/AssignmentTimephasedDataSet?$filter=ProjectId eq guid'" "79fdc49d-c0e3-e911-b09a-00155de03a0c" "'&$skip=" (str 0) "&$top=" (str max))]
  (as-> (-> (client/get url
                        (project/prepare-request-options))
            :body
            (json/read-str :key-fn csk/->kebab-case-keyword)
    ...
    (cske/transform-keys
      ...
      (mapv (fn [x] (let [task-name (:task/name x)
                      ...
                      (remove-nils
                       (assoc (dissoc x :task/name :task/id :task/is-active :project/id  :resource/id :task/start-date :task/end-date)
                         :assignment/task {:task/name task-name :task/id (uuid task-id) }
                         :assignment/resource resource)))) response)
